...
8. Store sensitive data via `fly secret`
...
## Lecture 7: October 17, 2025

### Fly Volumes

is covered at the beginning of L7



### Deployment Strategy

rolling, bluegreen, canary, immediate, is also covered at the beginning of L7



### Kubernetes

Open-sourced application, that managed container and scaling at a large scale, for the purpose of mid-to-large industry and business scale. Docker produce builds, images, and runs, K8s orchestrates.

#### K8s Architecture

- **Cluster**: A Kubernetes deployment made up of a **control plane** and multiple **worker nodes**
- **Control Plane**: Manages the worker nodes and Pods in the cluster
- **Worker Nodes**: Host Pods running containerized workloads
- **Pods**: Smallest deployable units, grouping one or more containers

![k8s](C:\Users\20838\Desktop\HalfBloodPrinceNotes\ECE1779 Cloud Computing\assets\k8s.svg)

#### K8s Control Plane

- **API Server (`kube-apiserver`)**: Frontend for Kubernetes API; entry point for all operations
- **`etcd`**: Key-value store for cluster state and configuration
- **Scheduler (`kube-scheduler`)**: Assigns Pods to nodes
- **Controller Managers**:
  - **`kube-controller-manager`**: Runs core controller processes
  - **`cloud-controller-manager`**: Runs cloud-specific controllers

![k8s-clusters](C:\Users\20838\Desktop\HalfBloodPrinceNotes\ECE1779 Cloud Computing\assets\k8s-clusters.svg)

#### K8s Worker Node Components

Worker nodes host pods, provide the resources and runtime for containers
...
- **`kube-proxy`**: Service routing component
  - Maintains network routing rules for Services

- [**CNI Plugin**](https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/): Pod networking provider
  - Allocates Pod IPs & connects Pods to cluster network
...
- A Pod groups one or more containers
- Containers in a Pod share
  - Network: Communicate via `localhost`
  - Storage: Share volumes
- Temporary and short-lived: Pods can fail and be replaced
- Example: A Pod running a single Nginx web server container

**Lifecycle**:

- **Pending**: Pod is accepted, waiting to start
- **Running**: Containers are up and running
- **Succeeded/Failed**: Containers completed (successfully or with error)
- **Unknown**: Status can’t be obtained

In practice: Pods are **rarely** created directly

- Kubernetes manages Pods (via controllers like **Deployment**) for resilience and scaling



#### K8s Service

**Service**: An abstraction that defines

- A logical group of Pods (by label selector)
- A policy by which to access Pods (stable IP and DNS name)
- Load balancing across Pods with the same label
...
Tools to run small-scale Kubernetes locally

#### Setup

Install minikube: [Link](https://minikube.sigs.k8s.io/docs/start/?arch=%2Fmacos%2Fx86-64%2Fstable%2Fbinary+download)
...
```sh
minikube start
kubectl get nodes  # list all worker nodes. If minikube, only one worker node
```
...
Pods network is not established by default. To make the application accessible from outside, do port forwarding
...
| Type             | Reachable From              | Example Access              |
| :--------------- | :-------------------------- | :-------------------------- |
| **ClusterIP**    | Inside cluster only         | `http://nginx-service:80`   |
| **NodePort**     | Outside via `<NodeIP>:port` | `http://192.168.1.10:30080` |
| **LoadBalancer** | External via cloud LB       | `http://203.0.113.5`        |
...
2. `doctl serverless install`: Install serverless Functions support
3. `doctl serverless status`: Verify installation
...
| :----------- | :------------------------- | -------------------------------------- |
| **Focus**    | Track known indicators     | Understand unknown issues              |
| **Data**     | Metrics, alerts            | Metrics, logs, traces                  |
| **Use Case** | Ensure uptime, performance | Debug distributed systems              |
| **Example**  | CPU > 70% alert            | Trace WebSocket latency across regions |



#### 3 Pillars of Observability:

- **Metrics**: Numeric time-series data for monitoring system health (Prometheus)
- **Logs**: Event records to understand what happened
- **Traces**: Follow requests across services to see how they flow
...
- **Grafana**: Visualizes + explores metrics

![metrics-example](assets/metrics-example.svg)
...
app.get("/", (req, res) => {
  res.send("Hello from Node with custom Prometheus metrics!");
...
Dashboard and visualization platform. Prometheus provides monitoring tool with basic UI, it does not have a lot of UI interactions. Grafana provides a comprehensive UI, including dashboard, heatmap, etc. So use Prometheus with Grafana.

1. Start Grafana through docker
    ```sh
    docker run -d -p 4000:3000 grafana/grafana
    ```

    If run locally in Docker, it uses local admin account not grafana.com account. 
    
    username / password: both admin, set a new local password

2. Add Prometheus:

   In Grafana: Connections → Data sources → Add new data source

   - Choose “Prometheus”
   - URL: `http://<droplet-ip>:9090`
   - Save & Test
...
3. Build Dashboard
   ...
   - Metric: `nodejs_heap_size_total_bytes`
   - Click “Run queries”
   - Adjust timescale
...
4. Clean up
   ...
   Check containes of Prometheus, Grafana
...
Like python logging, but when the scale of the containers rises, we need a platform to organize the logs.

#### ELK Stack

**ELK = Elasticsearch + Logstash + Kibana**, Industry Tools for Logs, Centralize, analyze, and visualize logs from all your systems and applications

- **Elasticsearch**: Store, index, and allow fast search on large volumes of log data
- **Logstash**: Process and ingest logs from multiple sources
  - Can transform data (e.g., filter, parse)
- **Kibana**: Visualization and exploration
  - Search logs, create dashboards, monitor metrics, build alerts

Workflow: Logs are collected by Logstash, stored and indexed with Elasticsearch, and Kibana visualized and everything



### Trace

Tell you which steps took how long

![observability](assets/observability.svg)
...
- **Service Models**: **IaaS** (DigitalOcean Droplets), **PaaS** (Vercel), **SaaS** (Notion)
- **Docker**: Containerization
- **Docker Compose**: Manage single-host multi-container Docker apps with `compose.yaml`
- **Docker Swarm**: Multi-host orchestration (services, stacks)
- **Fly.io**: Global edge deployment with auto-scaling
- **Kubernetes**: Industry-standard for large-scale, production-grade container orchestration
  - Concepts: Cluster, Control Plane, Worker Nodes, Pod, Service, ReplicaSet, Deployment
- **Serverless Computing**: **FaaS** (DigitalOcean Functions)
- **Edge Notifications**: Low-latency alerts via edge locations (DigitalOcean Function + Fly.io + WebSocket)
